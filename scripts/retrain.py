...
def create_model_graph(model_info):
  ...
  with tf.Graph().as_default() as graph:
    ...
    with gfile.FastGFile(model_path, 'rb') as f:
      ...
      bottleneck_tensor, resized_input_tensor = (tf.graph_util.import_graph_def(
          graph_def,
          name='',
          return_elements=[
...
def add_input_distortions(flip_left_right, random_crop, random_scale,
  ...
  decoded_image = tf.io.decode_jpeg(jpeg_data, channels=input_depth)
  ...
  resize_scale_value = tf.random_uniform(tensor_shape.scalar(),
                                         minval=1.0,
                                         maxval=resize_scale)
  scale_value = tf.math.multiply(margin_scale_value, resize_scale_value)
  precrop_width = tf.math.multiply(scale_value, input_width)
  precrop_height = tf.math.multiply(scale_value, input_height)
  ...
  brightened_image = tf.math.multiply(flipped_image, brightness_value)
  offset_image = tf.math.subtract(brightened_image, input_mean)
  mul_image = tf.math.multiply(offset_image, 1.0 / input_std)
  distort_result = tf.compat.v1.expand_dims(mul_image, 0, name='DistortResult')
...
def variable_summaries(var):
  ...
  with tf.name_scope('summaries'):
    ...
    with tf.name_scope('stddev'):
      stddev = tf.math.sqrt(tf.compat.v1.reduce_mean(tf.math.square(var - mean)))
...
def add_final_training_ops(class_count, final_tensor_name, bottleneck_tensor,
  ...
  with tf.name_scope(layer_name):
    ...
    with tf.name_scope('Wx_plus_b'):
      logits = tf.linalg.matmul(bottleneck_input, layer_weights) + layer_biases
...
def add_evaluation_step(result_tensor, ground_truth_tensor):
  ...
  with tf.name_scope('accuracy'):
    ...
    with tf.name_scope('accuracy'):
      evaluation_step = tf.compat.v1.reduce_mean(tf.cast(correct_prediction, tf.float32))
...
def add_jpeg_decoding(input_width, input_height, input_depth, input_mean,
  ...
  resized_image = tf.compat.v1.image.resize_bilinear(decoded_image_4d,
                                           resize_shape_as_int)
  offset_image = tf.math.subtract(resized_image, input_mean)
  mul_image = tf.math.multiply(offset_image, 1.0 / input_std)
...
if __name__ == '__main__':
  ...
  tf.compat.v1.compat.v1.app.run(main=main, argv=[sys.argv[0]] + unparsed)
